...
**Date:** 3 January 2026 (Updated 1:20 PM AEDT - Sync script fix applied)
...
| Sync script silent failure | ✅ Fixed | Fixed category name matching in `sync-burc-monthly.mjs` |

See:
- `docs/bug-reports/BUG-FIX-20260103-burc-critical-metrics-reconciliation.md`
- `docs/bug-reports/BUG-FIX-20260103-waterfall-sync-category-mismatch.md`
...
### Future Improvements (Priority 3) - ✅ ALL COMPLETED

6. **Consolidate Revenue Tables** ✅ COMPLETED (3 Jan 2026)
   - `burc_historical_revenue` (4 records, SA Health only)
   - `burc_historical_revenue_detail` (84,932 records)
   - **Finding:** Tables serve different purposes - detail table is a subset ($1.85M) vs historical table ($10.66M)
   - **Decision:** Cannot consolidate as detail table is incomplete; keeping separate tables

7. **Review CSI Licence Revenue** ✅ VERIFIED (3 Jan 2026)
   - `license_nr = $0` for 2026 months causes Sales Ratio = 0
   - **Finding:** Source file confirms $0 licence revenue for most months - only June ($19K) and July ($1.9M) have values
   - **Conclusion:** Database is correct - reflects actual performance, not missing data

8. **Automated BURC Sync Script** ✅ CREATED (3 Jan 2026)
   - Script: `scripts/sync-burc-monthly.mjs`
   - Features:
     - Reads 2026 APAC Performance.xlsx (forecast/actuals)
     - Syncs waterfall data (7 fields)
     - Syncs CSI OPEX data (12 monthly records)
     - Syncs attrition data
     - Dry-run mode for preview: `--dry-run`
     - Waterfall-only mode: `--waterfall`
   - Usage: `node scripts/sync-burc-monthly.mjs`
...
| `scripts/read-critical-burc-files.mjs` | Critical financial files analysis |
| `scripts/sync-burc-monthly.mjs` | **NEW** - Automated monthly BURC sync (waterfall, CSI, attrition) |
| `scripts/consolidate-revenue-tables.mjs` | Revenue tables analysis (confirmed tables serve different purposes) |
