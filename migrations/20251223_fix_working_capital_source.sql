-- Migration: Fix Working Capital Data Source
-- Date: 2025-12-23
-- Issue: Working Capital showing "No aging data" when data exists
-- Root Cause: View used wrong table (aged_accounts_receivable) and unidirectional alias lookup
--
...
-- Uses aging_accounts table (not aged_accounts_receivable) with bidirectional alias lookup
LEFT JOIN LATERAL (
  SELECT
    CASE
...
-- WHERE client_name ILIKE '%luke%' OR client_name ILIKE '%waikato%';
